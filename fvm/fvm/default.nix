{pkgs, components, contracts, support, ...}:
  ...
  support.buildRustPackage rec {
    ...
    ln -s ${components.file_open}/lib/libcomponent.so $out/bootstrap/${components.file_open.name}.so
    ln -s ${components.file_open}/lib/libcomponent.so bootstrap/${components.file_open.name}.so
    ln -s ${components.file_print}/lib/libcomponent.so $out/bootstrap/${components.file_print.name}.so
    ln -s ${components.file_print}/lib/libcomponent.so bootstrap/${components.file_print.name}.so
    ln -s ${components.development_fbp_parser_lexical}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_parser_lexical.name}.so
    ...
    ln -s ${components.development_fbp_parser_semantic}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_parser_semantic.name}.so
    ...
    ln -s ${components.development_fbp_parser_print_graph}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_parser_print_graph.name}.so
    ...
    ln -s ${components.development_capnp_encode}/lib/libcomponent.so $out/bootstrap/${components.development_capnp_encode.name}.so
    ln -s ${components.development_capnp_encode}/lib/libcomponent.so bootstrap/${components.development_capnp_encode.name}.so
    ln -s ${components.development_fbp_fvm}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_fvm.name}.so
    ln -s ${components.development_fbp_fvm}/lib/libcomponent.so bootstrap/${components.development_fbp_fvm.name}.so
    ln -s ${components.development_fbp_errors}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_errors.name}.so
    ln -s ${components.development_fbp_errors}/lib/libcomponent.so bootstrap/${components.development_fbp_errors.name}.so
    ln -s ${components.development_fbp_scheduler}/lib/libcomponent.so $out/bootstrap/${components.development_fbp_scheduler.name}.so
    ln -s ${components.development_fbp_scheduler}/lib/libcomponent.so bootstrap/${components.development_fbp_scheduler.name}.so
    ln -s ${support.component_lookup}/lib/libcomponent.so $out/bootstrap/${support.component_lookup.name}.so
    ln -s ${support.component_lookup}/lib/libcomponent.so bootstrap/${support.component_lookup.name}.so
    ln -s ${support.contract_lookup}/lib/libcomponent.so $out/bootstrap/${support.contract_lookup.name}.so
    ln -s ${support.contract_lookup}/lib/libcomponent.so bootstrap/${support.contract_lookup.name}.so

    ln -s ${pkgs.capnproto}/bin/capnp $out/bootstrap/capnp
