...
component! {
    nucleus_flow_parser_semantic, contracts(fbp_graph, fbp_lexical, fbp_semantic_error)
    ...
    fn run(&mut self) -> Result<()> {
        ...
        let literal: fbp_lexical::Reader = try!(ip.read_contract());
            ...
            fbp_lexical::Start(path) => {
                match handle_stream(&self)? {
                    ...
                    Err(errors) => {
                        ...
                        {
                            let mut ip = new_ip.build_contract::<fbp_semantic_error::Builder>();
                            ip.set_path(path?);
                            {
...
fn handle_stream(comp: &nucleus_flow_parser_semantic) -> Result<std::result::Result<Graph, Vec<String>>> {
    ...
    loop {
        ...
        let literal: fbp_lexical::Reader = ip.read_contract()?;
