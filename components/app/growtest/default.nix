{ stdenv, buildFractalideSubnet, upkeepers
  ...
  buildFractalideSubnet rec {
   ...
   lr(${ui_js_components.flex}) output -> places[1] td()
   '${js_create}:(type="div", style=[(key="display", val="flex")])~create' -> input lr()

   button_add(${ui_js_components.tag}) output -> places[1] lr()
   button_remove(${ui_js_components.tag}) output -> places[2] lr()
   '${js_create}:(type="button", text="add")~create' -> input button_add()
   '${js_create}:(type="button", text="remove")~create' -> input button_remove()

   dummy()

   gflex(${ui_js_components.growing_flex}) output -> places[2] td()
