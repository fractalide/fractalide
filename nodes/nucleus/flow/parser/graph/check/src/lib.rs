        ...
        let mut ip = try!(self.ports.recv("input"));
        {
            ...
            for n in try!(graph.borrow().get_edges()).iter() {
                if !nodes.contains_key(try!(n.get_o_name())) {
                    errors.push(format!("Uninstantiated node \"{0}()\"\n  Ensure you have included the type of the agent in this manner: \"{0}(${{agent_name}})\"", try!(n.get_o_name())));
                }
                if !nodes.contains_key(try!(n.get_i_name())) {
                    errors.push(format!("Uninstantiated node \"{0}()\"\n  Ensure you have included the type of the agent in this manner: \"{0}(${{agent_name}})\"", try!(n.get_i_name())));
                }
